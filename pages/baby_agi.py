...
class BabyAGI(BaseModel):
    ...
    def print_task_ending(self):
        st.success("Tasks completed based on iter. count.", icon="✅")
...
def main():
    ...
    if openai_api_key:
        OBJECTIVE = st.text_input(
            label=" 🏁 :blue[Ultimate Goal]: ",
            placeholder= "learn python fast",
            value="",
        )

        first_task = st.text_input(
            label=" :blue[Initial task:] ",
            value="Make a todo list",
        )
        # first_task = "Make a todo list"

        max_iterations = st.number_input(
            " :blue[Max Iterations]: ", value=3, min_value=1, step=1
        )
